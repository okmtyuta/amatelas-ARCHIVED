...
const baseCSS = /* css */ `
.AMUI_amatelas-button_.AMUI_amatelas-button_filled {
  outline: none;
  position: relative;
  overflow: hidden;
  padding: 12px 20px;
...
.AMUI_amatelas-button_.AMUI_amatelas-button_filled:disabled {
...
const colorVariations = () => {
  ...
  for (const [_color, _code] of Object.entries(color)) {
    ...
    _colorVariations.push(/* css */ `
      .AMUI_amatelas-button_.AMUI_amatelas-button_filled:not(:disabled).AMUI_amatelas-button_${_color} {
        border: none;
        color: white;
